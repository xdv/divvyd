...
divvyd_exe=/opt/divvy/bin/divvyd
conf_file=/etc/opt/divvy/divvyd.cfg
...
tmp_loc=$(mktemp -d --tmpdir divvy_info.XXXX)
cd /tmp
chmod 751 divvy_info.*
cd ~
echo ${tmp_loc}

cleaned_conf=${tmp_loc}/cleaned_divvyd_cfg.txt
...
echo "Use the following command on your local machine to download from your divvyd instance: scp <remote_divvyd_username>@<remote_host>:${tmp_loc}/info-package.tar.gz <path/to/local_machine/directory>"| tee /dev/fd/3
