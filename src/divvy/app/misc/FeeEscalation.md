...
Divvyd's fee mechanism consists of several interrelated processes:
...
for the queue if it meets these additional criteria:
  ...
  * all other queued transactions for that account, in the case where
  they spend the maximum possible XDV, leave enough XDV balance to pay
  the fee,
  * the total fees for the other queued transactions are less than both
  the network's minimum reserve and the account's XDV balance, and
...
This demonstrates that a simpler transaction paying less XDV can be more
likely to get into the open ledger, or be sorted earlier in the queue
than a more complex transaction paying more XDV.
...
more (or less) work for divvyd to process. Those transactions may have
...
behavior of the divvy network. Note that this is not necessarily the
...
automatically as the divvy network's performance improves, allowing
...
not make sense if divvyd is not synced to the network.
...
### [`server_info`](https://xdv.io/build/divvyd-apis/#server-info) command
...
### [`server_state`](https://xdv.io/build/divvyd-apis/#server-state) command
